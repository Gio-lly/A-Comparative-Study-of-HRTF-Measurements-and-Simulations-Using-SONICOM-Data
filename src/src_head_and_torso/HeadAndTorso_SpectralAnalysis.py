...
# Final comparison head-only / head+torso / SONICOM measurement.
# Update these paths if the files are in different folders.
files = {
    "sim_head_only": "/home/capstone/Downloads/P067/SOFA_P067_Right_merged/HRIR_SONICOM_grid_48000.sofa",
    "sim_head_torso": "/home/capstone/Downloads/P067/P067_torso_HRIR_SONICOM_grid_48000.sofa",
...
# Note: sim_head_torso - sim_head_only directly highlights the torso effect.
comparison_pairs = [
    ("sim_head_only", "mis_sonicom"),
    ("sim_head_torso", "mis_sonicom"),
    ("sim_head_torso", "sim_head_only"),
]

# FFT and display limits.
# For the head+torso analysis we use the 100-10000 Hz band,
# which is more robust against high-frequency differences related to fine notches,
# receiver, mesh and head-torso junction.
NFFT = 4096
FREQ_MIN_PLOT = 100.0
FREQ_MAX_PLOT = 10000.0
...
# automatically over all generated HRTF maps.
# If you want to set them manually, use for example:
# HRTF_COLOR_LIMITS_DB = (-40.0, 20.0)
HRTF_COLOR_LIMITS_DB = None

# Percentiles used to estimate the common scale of the absolute HRTF maps.
# Using percentiles avoids a few outliers forcing an overly wide colorbar.
COMMON_HRTF_COLOR_PERCENTILES = (1.0, 99.0)

# If DIFFERENCE_COLOR_LIMIT_DB = None, the common symmetric limit of the
# difference maps is estimated automatically over all differences.
# If you want to set it manually, use for example:
# DIFFERENCE_COLOR_LIMIT_DB = 30.0
DIFFERENCE_COLOR_LIMIT_DB = None

# Percentile used to estimate the common limit of the difference maps.
...
FREQ_MAX_METRIC = 10000.0

# ============================================================
# GLOBAL BROADBAND NORMALIZATION
# ============================================================

# If True, applies a global dB offset to some datasets before
# generating HRTF plots, difference maps and spectral metric CSVs.
# The goal is to remove a broadband gain offset between measurement and
# simulations, so as to compare mainly the direction-dependent spectral shape.
APPLY_BROADBAND_GAIN_NORMALIZATION = True

# For the head-only vs head+torso comparison, the measurement is normalized
# with respect to a neutral reference: the mean of sim_head_only and sim_head_torso.
# This way the normalization does not artificially favor either
# of the two simulations.
GAIN_NORMALIZATION_REFERENCE_MODE = "mean_of_datasets"  # options: "single_dataset", "mean_of_datasets"
GAIN_NORMALIZATION_REFERENCE_DATASET = "sim_head_only"  # used only if mode = "single_dataset"
GAIN_NORMALIZATION_REFERENCE_DATASETS = ["sim_head_only", "sim_head_torso"]
...
GAIN_NORMALIZATION_FREQ_HIGH = 6000.0
...
# To avoid generating too many images, local plot creation can be limited
...
output_root = script_dir / "plots_headonly_headtorso_measured_broadband_norm_10k"
...
def safe_filename(text):
    """
    Makes a string usable as a filename.
    """
    text = str(text)
    text = text.replace("+", "p")
    text = text.replace("-", "m")
...
def find_nearest_index(sofa, az_target, el_target):
    """
    Finds the index of the SOFA direction nearest to the target.
...
def hrir_energy_db(hrir):
    ...
    Used only for the left/right convention sanity check.
    """
...
def get_gain_reference_spectrum(hrtf_cache, set_name, target_label, ear):
    """
    Returns the reference spectrum in dB used to estimate
    the normalization offset.

    - single_dataset: uses a single dataset as reference;
    - mean_of_datasets: uses the dB mean of the indicated datasets.

    For the head-only/head+torso case, the recommended choice is mean_of_datasets
    with [sim_head_only, sim_head_torso], so the measurement is aligned to a
    neutral reference rather than to a single simulation.
    """
    if GAIN_NORMALIZATION_REFERENCE_MODE == "single_dataset":
        return hrtf_cache[set_name][target_label][GAIN_NORMALIZATION_REFERENCE_DATASET][ear]

    if GAIN_NORMALIZATION_REFERENCE_MODE == "mean_of_datasets":
        spectra = []
        for dataset_name in GAIN_NORMALIZATION_REFERENCE_DATASETS:
            spectra.append(hrtf_cache[set_name][target_label][dataset_name][ear])
        return np.mean(np.vstack(spectra), axis=0)

    raise ValueError(
        "GAIN_NORMALIZATION_REFERENCE_MODE deve essere 'single_dataset' oppure 'mean_of_datasets'."
    )


def get_gain_reference_label():
    if GAIN_NORMALIZATION_REFERENCE_MODE == "single_dataset":
        return GAIN_NORMALIZATION_REFERENCE_DATASET
    if GAIN_NORMALIZATION_REFERENCE_MODE == "mean_of_datasets":
        return "mean(" + "+".join(GAIN_NORMALIZATION_REFERENCE_DATASETS) + ")"
    return str(GAIN_NORMALIZATION_REFERENCE_MODE)
...
def estimate_broadband_gain_offsets(
    ...
    """
    Estimates a global broadband dB offset for each target dataset.
    ...
    The reference H_reference can be a single dataset or the mean
    across multiple datasets, according to GAIN_NORMALIZATION_REFERENCE_MODE.
    ...
    offsets_db = {}
    rows = []
    reference_label = get_gain_reference_label()
    ...
    for target_dataset in target_datasets:
        ...
        for set_name, targets in target_sets.items():
            for target_label, az_t, el_t in targets:
                for ear in [0, 1]:
                    H_ref = get_gain_reference_spectrum(hrtf_cache, set_name, target_label, ear)
                    ...
                    rows.append({
                        ...
                        "reference_mode": GAIN_NORMALIZATION_REFERENCE_MODE,
                        "reference_dataset_or_mean": reference_label,
                        "target_set": set_name,
...
def apply_broadband_gain_offsets(hrtf_cache, offsets_db):
    ...
    used for plots, differences and spectral metrics. The time-domain HRIRs
    remain raw/unnormalized.
    """
...
# Save CSV of the effective directions.
...
if APPLY_BROADBAND_GAIN_NORMALIZATION:
    ...
    reference_label = get_gain_reference_label()
    print(f"Reference mode: {GAIN_NORMALIZATION_REFERENCE_MODE}")
    ...
    for dataset_name, offset_db in gain_offsets_db.items():
        print(
            ...
            f"{offset_db:+.2f} dB "
            f"(rispetto a {reference_label})"
        )

    apply_broadband_gain_offsets(hrtf_cache, gain_offsets_db)

    # Saves both the local offsets used for the estimate and the final global offset.
...
def plot_hrtf_map(set_name, targets, dataset_name, ear, matrix):
    ...
    pcolormesh requires the X and Y axes to be edges if matrix C has
    shape (N_y, N_x). Therefore:
